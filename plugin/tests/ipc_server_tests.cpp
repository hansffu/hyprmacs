...
bool test_route_list_summon_candidates_escapes_control_chars() {
    hyprmacs::WorkspaceManager manager;
    auto applier = make_noop_applier();
    manager.seed_client("0xaaa", "1", "foot", "local", false);
    manager.seed_client("0xbbb", "2", "foot", std::string("remote") + char(0x01) + "shell", false);
    manager.manage_workspace("1");
    ...
    const hyprmacs::ProtocolMessage incoming {
        ...
        .workspace_id = "1",
        .timestamp = "2026-04-24T00:00:00Z",
        .payload_json = "{\"request_id\":\"req-ctrl\"}",
    };
    const auto responses = hyprmacs::route_command_for_tests(incoming, manager, applier);

    bool ok = true;
    ok &= expect(responses.size() == 1, "control-char summon candidate request should return one response");
    if (responses.size() == 1) {
        ok &= expect(responses[0].payload_json.find("\"request_id\":\"req-ctrl\"") != std::string::npos,
                     "summon-candidates response should echo request_id");
        ok &= expect(responses[0].payload_json.find("\\u0001") != std::string::npos,
                     "summon-candidates title should escape control char as unicode escape");
    }
    return ok;
}

bool test_route_list_summon_candidates_handles_escaped_request_id() {
    hyprmacs::WorkspaceManager manager;
    auto applier = make_noop_applier();
    manager.seed_client("0xaaa", "1", "foot", "local", false);
    manager.seed_client("0xbbb", "2", "foot", "remote", false);
    ...
    const hyprmacs::ProtocolMessage incoming {
        ...
        .workspace_id = "1",
        .timestamp = "2026-04-24T00:00:00Z",
        .payload_json = "{\"request_id\":\"req\\\"quoted\"}",
    };
    const auto responses = hyprmacs::route_command_for_tests(incoming, manager, applier);

    bool ok = true;
    ok &= expect(responses.size() == 1, "escaped request_id summon candidate request should return one response");
    if (responses.size() == 1) {
        ok &= expect(responses[0].payload_json.find("\"request_id\":\"req\\\"quoted\"") != std::string::npos,
                     "summon-candidates response should parse and re-escape quoted request_id");
    }
    return ok;
}
...
bool test_route_summon_client_escapes_control_chars_in_state_dump() {
    hyprmacs::WorkspaceManager manager([](const std::string&) {
        return 0;
    });
    auto applier = make_noop_applier();
    ...
    bool ok = true;
    ok &= expect(responses.size() == 2, "control-char summon-client should return ack and state-dump");
    if (responses.size() == 2) {
        ok &= expect(responses[1].type == "state-dump", "second response should be state-dump");
        ok &= expect(responses[1].payload_json.find("\\u0001") != std::string::npos,
                     "post-summon state-dump should escape control char as unicode escape");
    }
    return ok;
}
